...
def build_audit_json(events_rows, benefits_rows, performance_rows, insights_rows, monthly_rows, xlsx_meta):
    files = [
        ...
        {"file": "내부_골든위크_1~5·7월 xlsx 6개 (2026-09-15 추가)", "report_month": "N/A(이벤트 페이지 일별 로그)",
         "reliability": "높음(evidence A. 1~5월은 원본 월간보고서 자체가 없거나 정량 수치가 없던 달의 응모 실적을 새로 확인시켜줌. 7월분은 2026-09-04 값과 완전 일치로 재검증됨)"},
        {"file": "내부_11시콘서트_2·3·5·6·7·8월 xlsx 6개 (2026-09-15 추가, 8월분은 2026-09-04 중복)", "report_month": "N/A(이벤트 페이지 일별 로그)",
         "reliability": "높음(evidence A. 3월·6월분은 원래 이벤트 자체가 등록되어 있지 않던 달 - 원본 보고서 누락/유실을 메움)"},
        {"file": "내부_뉴스레터_6월_260615-260630.xlsx (2026-09-15 추가)", "report_month": "N/A(이벤트 페이지 일별 로그)",
         "reliability": "높음(evidence A, 뉴스레터는 응모고객 항목이 원본에 0으로 기재되어 있어 고객응모 KPI에는 기여하지 않는 채널로 확인됨)"},
        {"file": "카카오_5월 골든위크·불꽃어워즈(5~7월)·11시콘서트(6~8월)·뉴스레터(5~6월)·영업부스트패키지 xls 66개 (2026-09-15 추가)",
         "report_month": "N/A(카카오 채널 발송 로그)",
         "reliability": "중(evidence A, 단 메시지 템플릿마다 컬럼 구조가 11~28열로 5종류 달라 라벨 기반 파싱 필요 - 고정 열 위치로 읽으면 클릭수가 틀어짐을 검토 중 확인)"},
    ]
    ...
    audit = {
        ...
        "5_available_performance_metrics": [
            ...
            "[2026-09-15 추가] 골든위크 2026-01·02·03·04·05월, 11시콘서트 2026-02·03·05·06·07월 실제 응모(참여자) 수 - "
            "내부 실측 로그로 확인됨(evidence A). 이 중 골든위크 1월, 11시콘서트 3월·6월은 원본 월간보고서가 없거나 "
            "유실된 달의 이벤트를 새로 등록시켜준 유일한 근거.",
            "[2026-09-15 추가] 신규 카테고리 '뉴스레터'(2026-05·06월) 등록 - 카카오 채널 발송 로그 + 내부 로그로 "
            "노출/클릭/발송 실적 확보(evidence A). 응모고객 항목은 0으로 확인되어 고객응모 KPI에는 기여하지 않음.",
            "[2026-09-15 추가] 영업부스트 패키지(2026-06) - 카카오 채널 발송 로그로 '계획' 단계가 아니라 실제 "
            "집행되었음이 확인되어 data_status를 plan에서 mixed로 정정(evidence A, 노출/클릭수 확보, 참여자 수는 미확인)",
        ],
        ...
        "7_missing_fields": [
            ...
            "[해결됨, 참고] 이벤트별 참여자(응모) 수·방문자 수 - 골든위크(1~5·7월), 11시콘서트(2·3·5·6·7·8월 공연)는 "
            "2026-09-04/09-15 추가된 내부 로그로 확인됨. 터치어워즈(카카오 로그만 존재, 응모 데이터 없음), "
            "설계지원팀장, 원시트, 지방문화혜택 등은 여전히 누락.",
            "터치어워즈 2026-06 실행분 및 11시콘서트 2026-04 공연분(3월 응모) - 2026-09-15 추가 자료에도 해당 회차 "
            "로그가 없어(터치어워즈는 6월 pptx 자체가 유실, 11시콘서트는 '4월공연_3월응모' 폴더가 제공되지 않음) "
            "여전히 미확인.",
        ],
